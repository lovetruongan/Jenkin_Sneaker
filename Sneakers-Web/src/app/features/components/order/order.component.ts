...
export class OrderComponent extends BaseComponent implements OnInit,AfterViewInit {
  ...
  if (this.inforShipForm.invalid){
    ...
    const orderData = {
      ...
      total_money: this.finalCost + this.methodShippingValue.price, // Include shipping cost
    ...
    this.orderService.postOrder(orderData).pipe(
      tap((orderInfor: any) => {
        console.log('Order created response:', orderInfor); // Debug log
        this.orderId = orderInfor.id;
        this.commonService.orderId.next(orderInfor.id); 
        this.toastService.success("Đặt hàng thành công! Vui lòng thanh toán để hoàn tất đơn hàng.");
      }),
      switchMap(() => {
      ...
      tap(() => {
        ...
        // Delay một chút để đảm bảo order được tạo hoàn toàn
        setTimeout(() => {
          console.log('Navigating to order detail:', this.orderId); // Debug log
